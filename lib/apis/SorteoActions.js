...
        
        const campoBusqueda = coleccionRegistros === 'rifa2/general_registrations' 
...
    const resultados = { general: null, premium: null };
    ...
    // Sorteo General
    const ticketsG = await getTicketsFrom('rifa2/tickets_sold_general');
    const ganadorG = elegirGanador(ticketsG, "General");
    if (ganadorG) {
        resultados.general = await expandirDatosGanador(ganadorG, 'rifa2/general_registrations');
        resultados.general.ticketId = ganadorG.id; // Para mostrar el número ganado
    }

    // Sorteo Premium
    const ticketsP = await getTicketsFrom('rifa2/tickets_sold_premium');
    const ganadorP = elegirGanador(ticketsP, "Premium");
    if (ganadorP) {
        resultados.premium = await expandirDatosGanador(ganadorP, 'rifa2/premium_registrations');
...
export async function getSupporters() {
    try {
        await signInAnonymously(auth);
        const registrationsRef = collection(db, 'rifa2/general_registrations');
