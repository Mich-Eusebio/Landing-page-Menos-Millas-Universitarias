...
import { sendGenericText, sendNewLeadNotification } from './WhatsAppService';
...
export async function getSoldDays() {
    try {
        ...
        const formatName = (name) => {
            ...
            if (lower === 'anonymous' || lower === 'anonimous' || lower === 'anonimo' || lower === 'anónimo') {
                return 'Anónimo';
            }
            return name;
        };
        ...
        return snapshot.docs.map(document => {
            ...
            const morning = data.morning ? { ...data.morning, nombre: formatName(data.morning.nombre), link: data.morning.link || data.link || null } : null;
            const afternoon = data.afternoon ? { ...data.afternoon, nombre: formatName(data.afternoon.nombre), link: data.afternoon.link || data.link || null } : null;
            return {
                dateStr: document.id,
                nombre: formatName(data.nombre || data.sponsor_name || null),
                foto_url: data.foto_url || data.sponsor_photo_url || null,
                morning,
                afternoon,
                plan_seleccionado: data.plan_seleccionado || null,
                link: data.link || null,
            };
        });
    } catch (error) {
...
export async function saveWhatsAppLead(name, phone) {
    try {
        ...
        if (personalNumber) {
            console.log(`✉️ Enviando notificación de lead (template) a: ${personalNumber}`);
            await sendNewLeadNotification({
                telefono: personalNumber,
                nombre: name
            });
        } else {
            console.warn("⚠️ PERSONAL_NUMER / PERSONAL_NUMBER no está definido en las variables de entorno.");
        }
...
export async function savePdfDownloadLead(email) {
    try {
        await signInAnonymously(auth);
        const leadsRef = collection(db, 'lead-capture');
        await addDoc(leadsRef, {
        ...
        const personalNumber = process.env.PERSONAL_NUMER || process.env.PERSONAL_NUMBER;
        if (personalNumber) {
            console.log(`✉️ Enviando notificación de lead PDF a: ${personalNumber}`);
            await sendNewLeadNotification({
                telefono: personalNumber,
                nombre: `PDF Lead: ${email}`
            });
        }

        return { success: true };
    } catch (error) {
        console.error("🔥 Error al guardar lead de descarga PDF:", error);
        throw error;
    }
}


export async function sendRafflePurchaseNotification(name, planName) {
    try {
        const personalNumber = process.env.PERSONAL_NUMER || process.env.PERSONAL_NUMBER;
        if (personalNumber) {
            console.log(`✉️ Enviando notificación de compra de rifa a: ${personalNumber}`);
            const messageParam = `${name} ha comprado ${planName} y fue notificado.`;
            await sendNewLeadNotification({
                telefono: personalNumber,
                nombre: messageParam
            });
        ...
        console.error("🔥 Error al enviar notificación de rifa por WhatsApp:", error);
        throw error;
    }
}

export async function sendRaffleNotificationStatus(name, wasNotified) {
    try {
        const personalNumber = process.env.PERSONAL_NUMER || process.env.PERSONAL_NUMBER;
        if (personalNumber) {
            console.log(`✉️ Enviando notificación de estado de WhatsApp a Michael: ${personalNumber}`);
            const messageParam = wasNotified 
                ? `${name} fue notificada de sus tickets`
                : `${name} NO fue notificada de sus tickets (falló envío)`;
            await sendNewLeadNotification({
                telefono: personalNumber,
                nombre: messageParam
            });
        ...
        console.error("🔥 Error al enviar notificación de estado de WhatsApp a Michael:", error);
        throw error;
    }
}
