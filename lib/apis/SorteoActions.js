...
import { collection, query, where, limit, getDocs, addDoc, deleteDoc, doc, getDoc, setDoc } from 'firebase/firestore';
...
export async function getSoldDays() {
    try {
        ...
        return snapshot.docs.map(document => ({
            dateStr: document.id,           // El ID del doc es la fecha yyyy-mm-dd
            nombre: document.data().nombre || null,
            foto_url: document.data().foto_url || null,
 ...
 * @param {string|null} foto_url - URL de la foto del patrocinador (HU2)
 * @param {string}   plan        - Nombre del tier seleccionado
 */
export async function saveSoldDays({ dates, nombre, foto_url, plan }) {
    try {
        await signInAnonymously(auth);
        const writes = dates.map(dateStr => {
            const docRef = doc(db, 'sold-days', dateStr);
...
export async function saveNewsletterSubscription(payload) {
    try {
        const { email, monthsSubscribed, name, comprobante_url } = payload;
