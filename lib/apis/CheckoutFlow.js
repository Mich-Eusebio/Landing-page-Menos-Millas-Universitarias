...
export default function CheckoutFlow({ selectedDays, isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('🔍 [Checkout] selectedDays:', selectedDays)
      console.log('🔍 [Checkout] formData:', formData)

      const latestSoldDays = await getSoldDays()
      console.log('🔍 [Checkout] latestSoldDays obtenido:', latestSoldDays.length, 'días')
      ...
      if (conflicted.length > 0) {
        console.warn('⚠️ [Checkout] Conflictos detectados:', conflicted)
      ...
      if (formData.photoFile) {
        console.log('🔍 [Checkout] Subiendo foto del sponsor...')
        ...
        console.log('✅ [Checkout] Foto del sponsor subida:', sponsor_foto_url)
      }

      let comprobante_url = null
      if (formData.proof) {
        console.log('🔍 [Checkout] Subiendo comprobante...')
        ...
        console.log('✅ [Checkout] Comprobante subido:', comprobante_url)
      }

      const plan_seleccionado = getTierId(n)
      const uid = Math.random().toString(36).substring(2, 9)

      const comprameUnDiaPayload = {
      ...
      console.log('🔍 [Checkout] Guardando en comprame-un-dia...', comprameUnDiaPayload)
      await saveComprameUnDia(comprameUnDiaPayload)
      console.log('✅ [Checkout] comprame-un-dia guardado exitosamente')

      const soldDaysPayload = {
        dates: selectedDays,
      ...
      console.log('🔍 [Checkout] Guardando sold-days...', soldDaysPayload)
      await saveSoldDays(soldDaysPayload)
      console.log('✅ [Checkout] sold-days guardado exitosamente')
      ...
      console.log('✅ [Checkout] Proceso completado, redirigiendo a /gracias')
      window.location.href = '/gracias'
    } catch (error) {
      console.error("❌ [Checkout] Error en handleSubmit:", error)
