...
export async function sendCheckoutEmail(formData) {
  ...
  try {
    // Al estar en un Server Action (Node.js), usamos fetch directamente a la API de EmailJS
    // para evitar errores de entorno (browser vs server) manteniendo la misma lógica.
